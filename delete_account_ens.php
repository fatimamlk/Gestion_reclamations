...
<div class="container">
    <form class="mt-4" action="delete_account_ens.php" method="post">
        <h1>Supprimer un compte enseignant</h1>
        <div class="form-group col-6 mx-auto">
            <label for="id">CIN de l'enseignant :</label>
            <input type="text" class="form-control d-block" name="id" required>
        </div>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if (isset($_POST["id"])) {
            ...
            if ($result && mysqli_num_rows($result) > 0) {
                ...
                $sql2 = "DELETE FROM enseignant WHERE CIN = '$id'";
                $sql3 = "DELETE FROM user WHERE user_id = '$userid'";

                if (mysqli_query($con, $sql2) && mysqli_query($con, $sql3)) {
                    ?> <script>
            alert("Le compte enseignant a été supprimé avec succès. ");
            history.go(-1);
          </script>
          ...
          <?php
                   ...
                   mysqli_error($con);
                    ?> <script>
            alert("Erreur lors de la suppression du compte enseignant :  ");
            history.go(-1);
          </script>
          ...
          <?php
                ...
                echo "Aucun enseignant trouvé avec cet ID.";
            }
